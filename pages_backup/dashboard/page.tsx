...
const LazyAdvancedCallAnalytics = React.lazy(() => import('../components/AdvancedCallAnalytics'))
const LazyCallPlayer = React.lazy(() => import('../components/CallPlayer'))
const LazyAIInsights = React.lazy(() => import('../components/AIInsights'))
...
export default function Dashboard() {
  ...
  const [realMetrics, setRealMetrics] = useState<unknown>(null)
  const [realActivity, setRealActivity] = useState<unknown[]>([])
  ...
  const [filters, setFilters] = useState<unknown>({ status: [], serviceType: [], dateRange: '' })
  ...
  const handleLogout = () => {
    ...
    document.cookie = 'token=; path=/; expires=Thu, 01 Jan 1970 00: 00 GMT'
        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            >
              ...
              {dashboardData?.recentCalls && dashboardData.recentCalls.length > 0 ? (
                <div className="space-y-3">
                  {dashboardData.recentCalls.slice(0, 3).map((call: unknown) => (
  ...
  filter: 'invert(0.9) hue-rotate(180deg)',
          ...
          <motion.div
            ...
            className="mb-8"
          >
            <Suspense fallback={<div className="h-96 bg-gray-800/50 rounded-xl animate-pulse" />}>
              <ErrorBoundary>
                <LazyAdvancedCallAnalytics 
                  businessId={dashboardData?.businessId || ''} 
                />
              </ErrorBoundary>
            </Suspense>
          ...
          {/* Call Player - Show for most recent call */}
          {dashboardData?.recentCalls && dashboardData.recentCalls.length > 0 && (
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              <Suspense fallback={<div className="h-96 bg-gray-800/50 rounded-xl animate-pulse" />}>
                <ErrorBoundary>
                  <LazyCallPlayer 
                    callId={dashboardData.recentCalls[0].id}
                    businessId={dashboardData?.businessId || ''} 
                  />
                </ErrorBoundary>
              </Suspense>
