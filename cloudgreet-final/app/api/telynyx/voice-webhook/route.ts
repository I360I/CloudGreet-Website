...
export async function POST(request: NextRequest) {
  try {
    ...
    if (businessError || !business) {
      logger.error('Error finding business for webhook', businessError, { to })
      return NextResponse.json({
        call_id: call_control_id,
        status: 'answered',
        instructions: [
          { instruction: 'say', text: 'Thank you for calling. We are currently unavailable. Please try again later.', voice: 'alice' },
          { instruction: 'hangup' }
        ]
      })
    }
    ...
    if (callError) {
      logger.error('Error storing call', callError, { body })
    }
    ...
    if (agentError || !agent) {
      logger.error('Error finding active agent', agentError, { businessId: business.id })
      return NextResponse.json({
        call_id: call_control_id,
        status: 'answered',
        instructions: [
          { instruction: 'say', text: `Thank you for calling ${business.business_name}. We are currently unavailable. Please try again later.`, voice: 'alice' },
          { instruction: 'hangup' }
        ]
      })
    }
    ...
    logger.error('Voice webhook error', error as Error, { endpoint: 'voice_webhook', body: await request.json().catch(() => ({})) })
    return NextResponse.json({
      call_id: 'unknown',
      status: 'error',
      instructions: [
