...
export default function OnboardingWizard({ isOpen, onClose, onComplete }: OnboardingWizardProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    // AI Configuration
    greetingMessage: `Hello! Thank you for calling ${getInitialData().businessName || '[Business Name]'}. How can I help you today?`,
    tone: 'professional',
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      if (!formData.businessName || !formData.ownerName || !formData.email || !formData.phone) {
        alert('Please fill in all required fields before completing onboarding.')
      ...
      if (!formData.greetingMessage) {
        alert('Please provide a greeting message for your AI receptionist.')
        setIsSubmitting(false)
        return
      }
      ...
      if (response.ok) {
        ...
        alert(`Onboarding failed: ${errorData.message || 'Unknown error'}`)
      }
    } catch (error) {
      // Console error removed for production
          ...
          <div className="border-t border-gray-700/50 p-6">
            <div className="flex items-center justify-between">
              ...
              <motion.button
                ...
                className="flex items-center space-x-2 px-8 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-lg font-medium hover:from-blue-700 hover:to-purple-700 transition-all shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isSubmitting ? (
                  <>
