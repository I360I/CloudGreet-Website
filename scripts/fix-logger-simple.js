...
console.log('🔧 Simple fix for logger patterns...\n');
...
for (const file of filesToFix) {
  try {
    ...
    // Just find and replace the .replace pattern fragments
    while (content.includes('error instanceof Error ? error.message.replace(/[<>]/g, \'\'')) {
      content = content.replace(
        /error instanceof Error \? error\.message\.replace\(\/\[<>\]\/g, ''\s+/g,
        "error instanceof Error ? error.message "
      );
    }

    // Also fix any remaining patterns with the weird nested structure
    content = content.replace(
      /instanceof Error \? \{[^}]*error:\s+error\s+instanceof\s+Error\s+\?\s+error\.message[^}]*\}\s+:\s+'Unknown\s+error'/g,
      ": 'Unknown error'"
    );
