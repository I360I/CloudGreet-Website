...
export async function getCalendarConfig(businessId: string): Promise<CalendarConfig | null> {
  try {
    ...
    if (error) {
      logger.error('Failed to fetch calendar config', error, { businessId })
      return null
    }
    ...
    logger.error('Calendar config fetch error', error as Error, { businessId })
    return null
  }
}
...
export async function createCalendarEvent(businessId: string, event: Omit<CalendarEvent, 'id'>): Promise<CalendarEvent | null> {
  try {
    ...
    if (error) {
      logger.error('Failed to create calendar event', error, { businessId, event })
      return null
    }
    ...
    logger.error('Calendar event creation error', error as Error, { businessId, event })
    return null
  }
}
...
export async function getAvailableSlots(businessId: string, date: string, duration: number = 60): Promise<string[]> {
  try {
    ...
    if (error) {
      logger.error('Failed to fetch appointments for slot calculation', error, { businessId, date })
      return []
    }
    ...
    logger.error('Available slots calculation error', error as Error, { businessId, date, duration })
    return []
  }
}
