...
export async function recordPerformanceMetrics(
  ...
  try {
    ...
    if (error) {
      logger.error('Failed to record performance metrics', error, { endpoint, metrics })
    }
  } catch (error) {
    logger.error('Performance metrics recording error', error as Error, { endpoint, metrics })
  }
}
...
export async function getPerformanceMetrics(
  ...
  try {
    ...
    if (error) {
      logger.error('Failed to fetch performance metrics', error, { endpoint, hours })
      return []
    }
    ...
    logger.error('Performance metrics fetch error', error as Error, { endpoint, hours })
    return []
  }
}
...
export async function recordSystemHealth(health: Omit<SystemHealth, 'lastHealthCheck'>): Promise<void> {
  try {
    ...
    if (error) {
      logger.error('Failed to record system health', error, { health })
    }
  } catch (error) {
    logger.error('System health recording error', error as Error, { health })
  }
}
