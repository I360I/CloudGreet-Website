...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (appointmentError) {
      logger.error('Error updating appointment', appointmentError, { appointmentId, businessId })
    ...
    if (sendReviewSMS) {
      try {
        ...
        if (reviewResponse.ok) {
          ...
          logger.error('Failed to send review SMS', null, { appointmentId, businessId })
        }
      } catch (error) {
    ...
    logger.error('Complete appointment error', error as Error, { 
