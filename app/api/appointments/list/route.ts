...
export async function GET(request: NextRequest) {
  try {
    ...
    if (appointmentsError) {
      logger.error('Error fetching appointments', appointmentsError, { businessId, userId })
    ...
    if (countError) {
      logger.error('Error fetching appointment count', countError, { businessId })
    }
    ...
    logger.error('Appointments list API error', error as Error, {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (appointmentError) {
      logger.error('Error creating appointment', appointmentError, { businessId, userId })
    ...
    logger.error('Create appointment API error', error as Error, {
      userId: request.headers.get('x-user-id'),
      businessId: request.headers.get('x-business-id')
    })
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
