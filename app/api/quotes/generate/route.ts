...
export async function POST(request: NextRequest) {
  try {
    ...
    if (rulesError) {
      logger.error('Error fetching pricing rules', rulesError, { 
    ...
    if (quoteError) {
      logger.error('Error saving quote', quoteError, { 
        businessId: validatedData.business_id,
    ...
    logger.error('Quote generation API error', error as Error, { 
...
Keep the tone professional but friendly, and be specific about what the estimate includes.`
    ...
    logger.error('AI quote generation error', error as Error, {
