...
export async function POST(request: NextRequest) {
  try {
    ...
    if (appointmentError || !appointment) {
      logger.error('Appointment not found', appointmentError, { appointmentId, businessId })
    ...
    if (businessError || !business) {
      logger.error('Business not found', businessError, { businessId })
...
Reply STOP to opt out; HELP for help.`
    ...
    if (smsError) {
      logger.error('Error storing SMS in database', smsError, { appointmentId, businessId })
    ...
    logger.error('Send review SMS error', error as Error, { 
