...
export async function POST(request: NextRequest) {
  try {
    ...
    if (voicemailError) {
      logger.error('Failed to store voicemail', voicemailError, {
        call_id,
        recording_url
      })
    }
    ...
    if (voicemailRecord) {
      try {
        ...
        if (!notificationResponse.ok) {
          logger.error('Failed to send voicemail notification', new Error('Notification API failed'), {
            call_id,
            status: notificationResponse.status
          })
        }
      } catch (notificationError) {
        logger.error('Error sending voicemail notification', notificationError as Error, {
          call_id
        })
      }
    }
    ...
    logger.error('Voicemail handler error', error as Error, {
