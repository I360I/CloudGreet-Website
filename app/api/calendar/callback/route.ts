...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      logger.error('Google Calendar OAuth error', new Error(error), {
        error,
        state
      })
    ...
    if (!tokenResponse.ok) {
      ...
      logger.error('Failed to exchange Google OAuth code', new Error(errorData), {
    ...
    if (businessError || !business) {
      logger.error('Business not found for calendar integration', businessError, {
        businessName: state
      })
    ...
    if (updateError) {
      logger.error('Failed to store Google Calendar credentials', updateError, {
    ...
    logger.error('Calendar callback error', error as Error, {
