...
import {
 LayoutGrid, ListChecks, Trophy, DollarSign, LogOut, Loader2,
} from 'lucide-react'
import { fetchWithAuth } from '@/lib/auth/fetch-with-auth'

type ActiveLabel = 'Overview' | 'Leads' | 'Closes' | 'Earnings'

const NAV: { label: ActiveLabel; href: string; icon: React.ElementType; match: (p: string) => boolean }[] = [
 { label: 'Overview',  href: '/sales',          icon: LayoutGrid,  match: (p) => p === '/sales' },
 ...
 { label: 'Earnings',  href: '/sales/earnings', icon: DollarSign,  match: (p) => p.startsWith('/sales/earnings') },
]

/**
 * Layout for the rep-side dashboard. Same visual language as the
 * client dashboard (light, paper-cream background) so reps and clients
 * have a consistent look. Distinct from the admin Shell which is dark.
 */
export function SalesShell({
 ...
 return (
  <main className="min-h-screen bg-[#f6f5f1] text-gray-900 flex">
   <aside className="hidden lg:flex w-60 border-r border-black/5 flex-col py-6 px-4 sticky top-0 h-screen">
    ...
    <div className="border-t border-black/5 pt-4 px-2 space-y-3">
     ...
     {payoutsReady === false && (
      <Link
       ...
       className="block bg-amber-50 border border-amber-200 rounded-lg px-3 py-2"
      >
