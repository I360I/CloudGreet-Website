  ...
  const handleManageBilling = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showSuccess('Billing Portal Opened', 'You can now manage your subscription and payment methods')
      } else {
        const errorData = await response.json()
        showError('Failed to Open Billing Portal', errorData.message || 'Unknown error occurred')
      }
    } catch (error) {
      console.error('Billing portal error:', error)
      showError('Billing Portal Error', 'Failed to open billing portal. Please try again or contact support.')
    } finally {
      setIsLoading(false)
    }
  }
