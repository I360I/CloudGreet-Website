...
export default function ChatGPTVoice({
  ...
  const connectToOpenAI = useCallback(async () => {
    try {
      addDebugInfo('Connecting to OpenAI Realtime API...')
      ...
      wsRef.current.onopen = () => {
        ...
        // Send session configuration for natural voice
        const sessionConfig = {
          type: "session.update",
          session: {
      ...
      wsRef.current.onmessage = (event) => {
        try {
          ...
          if (data.type === 'response.audio.delta') {
            // Handle audio response - this is the natural voice
            const audioData = data.delta
            if (audioData) {
              playNaturalAudio(audioData)
            }
  ...
  // Play natural audio from OpenAI (not robotic)
  const playNaturalAudio = useCallback(async (audioData: string) => {
    ...
    try {
      ...
      addDebugInfo('Playing natural OpenAI audio')
      setIsSpeaking(true)
    } catch (error) {
  ...
  const connect = useCallback(async () => {
    try {
      setError(null)
      setDebugInfo([])
      addDebugInfo('Starting ChatGPT-style voice AI...')
      ...
      addDebugInfo('ChatGPT-style voice AI connected successfully')
      console.log('✅ ChatGPT-style voice AI ready')
  ...
  const disconnect = useCallback(() => {
    addDebugInfo('Disconnecting from ChatGPT-style voice AI...')
    ...
    setCurrentMessage('')
    setError(null)
    addDebugInfo('Disconnected from ChatGPT-style voice AI')
  }, [addDebugInfo])
        ...
        <AnimatePresence mode="wait">
          {orbState === 'idle' && (
            <motion.div key="idle" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <h2 className="text-2xl font-bold mb-2 text-white">
                Connect to ChatGPT Voice
              </h2>
              <p className="text-gray-400">Click to start NATURAL AI conversation</p>
            </motion.div>
          )}
          ...
          {orbState === 'speaking' && (
            <motion.div key="speaking" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              ...
              <p className="text-gray-400">OpenAI is responding with natural voice</p>
            </motion.div>
          )}
      ...
      {debugInfo.length > 0 && (
        ...
        >
          <div className="bg-gray-900/40 backdrop-blur-sm border border-gray-400/20 rounded-xl p-4">
            <h3 className="text-white text-sm font-bold mb-2">ChatGPT Voice Debug:</h3>
