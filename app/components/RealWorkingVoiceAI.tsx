...
interface RealWorkingVoiceAIProps {
  businessName: string
  businessType: string
  services: string
  hours: string
...
const RealWorkingVoiceAI: React.FC<RealWorkingVoiceAIProps> = ({
  businessName,
  businessType,
  services,
  hours,
  ...
  const initializeAudio = useCallback(async () => {
    try {
      console.log('🎤 Initializing real audio system...')
  ...
  const connect = useCallback(async () => {
    try {
      console.log('🚀 Starting REAL working voice system...')
      ...
      } else if (data.clientSecret && typeof data.clientSecret === 'object' && data.clientSecret.value) {
        clientSecret = data.clientSecret.value
      } else {
        clientSecret = JSON.stringify(data.clientSecret)
      }
      
      // Use server-side proxy for authentication since browsers can't send custom headers
      console.log('🔌 Creating server-side WebSocket proxy...')
      
      const proxyResponse = await fetch('/api/voice/websocket-server', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      console.log('🔌 Server proxy response:', proxyData)

      if (!proxyData.success) {
        throw new Error(`Server proxy failed: ${proxyData.error}`)
      }

      console.log('✅ Server-side WebSocket proxy connected')
      
      // Create a mock WebSocket that communicates with our server proxy
      const mockWs = {
        readyState: WebSocket.OPEN,
        send: async (data: any) => {
          console.log('📤 Sending via server proxy:', data)
          const response = await fetch('/api/voice/websocket-server', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
        ...
        close: async () => {
          console.log('🔌 Closing server proxy connection')
          await fetch('/api/voice/websocket-server', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
      ...
      // Simulate WebSocket connection events for mock WebSocket
      console.log('✅ Connected to REAL OpenAI Realtime API via server proxy')
      ...
      await ws.send(JSON.stringify(sessionConfig))
      console.log('🔐 Sent session configuration')
      
      // Send greeting to trigger AI response
      ...
      await ws.send(JSON.stringify(greetingMessage))
  ...
  const startListening = useCallback(() => {
    if (!wsRef.current || wsRef.current.readyState !== WebSocket.OPEN) {
      setError('Not connected to OpenAI')
      return
    }
    
    console.log('🎤 Starting real listening...')
    setIsListening(true)
    setStatus('Listening...')
  }, [])
...
export default RealWorkingVoiceAI
