...
interface ServerSideRealtimeVoiceProps {
  businessName: string
  businessType: string
  services: string
  hours: string
...
const ServerSideRealtimeVoice: React.FC<ServerSideRealtimeVoiceProps> = ({
  businessName,
  businessType,
  services,
  hours,
  ...
  // Process audio through server-side realtime proxy
  const processAudio = useCallback(async (audioBlob: Blob) => {
    if (isProcessingRef.current) return
    isProcessingRef.current = true

    try {
      console.log('🎤 Processing audio through server-side realtime proxy...')
      ...
      // Send to server-side realtime proxy
      const response = await fetch('/api/voice/proxy-stream/' + sessionIdRef.current, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(`Server-side realtime processing failed: ${errorData.error}`)
      }

      const data = await response.json()
      console.log('✅ Server-side realtime audio processed:', data.message)
      
      // For now, simulate a response since the proxy isn't fully implemented
      const mockResponse = {
        content: "Hello! I'm your AI receptionist. I heard your message. How can I help you today?",
        audio: null
      }
      ...
      console.error('❌ Server-side realtime processing error:', error)
      setError(`Server-side realtime processing failed: ${error.message}`)
    } finally {
  ...
  const connect = useCallback(async () => {
    try {
      console.log('🚀 Starting server-side realtime voice system...')
      setError(null)
      ...
      setIsConnected(true)
      setStatus('Connected')
      console.log('✅ Server-side realtime voice system connected')
  ...
  const disconnect = useCallback(() => {
    console.log('🛑 Disconnecting server-side realtime voice system...')
    ...
    isProcessingRef.current = false
    console.log('✅ Server-side realtime voice system disconnected')
  }, [])
...
export default ServerSideRealtimeVoice
