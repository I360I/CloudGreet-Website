...
interface ProperRealtimeVoiceProps {
  businessName?: string
  businessType?: string
  services?: string
  hours?: string
...
export default function ProperRealtimeVoice({
  ...
  const connectToOpenAI = useCallback(async () => {
    try {
      addDebugInfo('Creating OpenAI session...')
      
      // Create session first
      const sessionResponse = await fetch('/api/voice/realtime-session', {
        method: 'POST',
        headers: {
      ...
      addDebugInfo(`Session created: ${sessionData.session_id}`)

      // Connect to WebSocket with session credentials
      const wsUrl = `wss://api.openai.com/v1/realtime?session_id=${sessionData.session_id}&client_secret=${sessionData.client_secret}`
  ...
  const connect = useCallback(async () => {
    try {
      setError(null)
      setDebugInfo([])
      addDebugInfo('Starting proper OpenAI Realtime API connection...')
