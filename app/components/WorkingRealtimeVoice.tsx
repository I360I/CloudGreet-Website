...
interface WorkingRealtimeVoiceProps {
  businessName: string
  businessType: string
  services: string
  hours: string
...
const WorkingRealtimeVoice: React.FC<WorkingRealtimeVoiceProps> = ({
  businessName,
  businessType,
  services,
  hours,
  ...
  // Process audio through server-side realtime
  const processAudio = useCallback(async (audioBlob: Blob) => {
    if (isProcessingRef.current) return
    isProcessingRef.current = true

    try {
      console.log('🎤 Processing audio through server-side realtime...')
      ...
      const response = await fetch('/api/voice/realtime-audio', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(`Realtime processing failed: ${errorData.error}`)
      }

      const data = await response.json()
      console.log('✅ Realtime audio processed:', data.response.content)
      
      // Add to conversation
      setConversationHistory(prev => [...prev, 
        { role: 'user', content: data.transcription || '[Audio message]' },
      ...
      if (data.response.audio) {
        setIsSpeaking(true)
        try {
          const audioData = Uint8Array.from(atob(data.response.audio), c => c.charCodeAt(0))
      ...
      setError(`Realtime processing failed: ${error.message}`)
    } finally {
  ...
  const connect = useCallback(async () => {
    try {
      console.log('🚀 Starting WORKING Realtime voice system...')
      setError(null)
      ...
      setIsConnected(true)
      setStatus('Connected')
      console.log('✅ WORKING Realtime voice system connected')
  ...
  const disconnect = useCallback(() => {
    console.log('🛑 Disconnecting WORKING realtime voice system...')
    ...
    isProcessingRef.current = false
    console.log('✅ WORKING realtime voice system disconnected')
  }, [])
...
export default WorkingRealtimeVoice
