...
const RealtimeStreamingVoice: React.FC<RealtimeStreamingVoiceProps> = ({
  businessName,
  businessType,
  services,
  hours,
  ...
  const initializeAudio = useCallback(async () => {
    try {
      console.log('🎤 Initializing audio for REAL realtime streaming...')
      ...
      console.log('🎤 Audio initialized for REAL realtime streaming')
      return true
    } catch (err: any) {
  ...
  // Connect to OpenAI Realtime API through server-side WebSocket
  const connectToOpenAI = useCallback(async () => {
    try {
      ...
      console.log('🔌 Creating server-side REAL Realtime WebSocket...')
      
      // Create server-side WebSocket connection
      const wsResponse = await fetch('/api/voice/realtime-websocket', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!wsResponse.ok) {
        ...
        throw new Error(`REAL Realtime WebSocket failed: ${errorData.error}`)
      }

      const wsData = await wsResponse.json()
      console.log('✅ REAL Realtime WebSocket ready:', wsData.message)
      
      // Now connect to OpenAI Realtime API directly with session credentials
      console.log('🔌 Connecting to OpenAI Realtime API for streaming...')
      ...
      ws.onopen = () => {
        console.log('✅ Connected to OpenAI Realtime API for streaming')
        ...
        const sessionConfig = {
          ...
          session: {
            ...
            instructions: `You are a professional AI receptionist for ${businessName}. Be helpful, friendly, and professional. Keep responses concise and natural for voice conversation.`,
            voice: 'alloy',
        ...
        console.log('🔐 Sent realtime streaming configuration')
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (data.type === 'response.audio.delta') {
            ...
            console.log('🎤 Realtime audio input committed')
          } else if (data.type === 'conversation.item.output_audio_buffer.committed') {
            console.log('🔊 Realtime audio output committed')
          }
        } catch (error) {
          console.error('❌ Error parsing Realtime WebSocket message:', error)
        }
      }
      ...
      ws.onclose = (event) => {
        ...
        setIsSpeaking(false)
        if (event.code !== 1000) {
          setError(`Realtime connection closed unexpectedly: ${event.reason || event.code}`)
        }
      }
    } catch (error: any) {
      console.error('❌ Realtime connection error:', error)
      setError(`Realtime connection failed: ${error.message}`)
      setIsConnected(false)
      setIsListening(false)
      setIsSpeaking(false)
    }
  ...
  const startListening = useCallback(() => {
    if (!wsRef.current || wsRef.current.readyState !== WebSocket.OPEN) {
      setError('Realtime WebSocket not connected.')
      return
    }
  ...
  const stopListening = useCallback(() => {
    ...
    console.log('🔇 Stopping realtime listening...')
    setIsListening(false)
  ...
  const connect = useCallback(async () => {
    try {
      console.log('🚀 Starting REAL Realtime streaming voice system...')
      setError(null)
  ...
  const disconnect = useCallback(() => {
    console.log('🛑 Disconnecting REAL Realtime streaming voice system...')
    if (wsRef.current) {
      wsRef.current.close()
    }
    ...
    isPlayingAudioRef.current = false
    console.log('✅ REAL Realtime streaming voice system disconnected')
  }, [])
...
export default RealtimeStreamingVoice
