...
/**
 * Shared lead-workspace UI - the table/filters/bulk-actions/scraper-entry
 * content that used to live only in app/sales/leads/page.tsx. Rendered by
 * both /sales/leads (sales reps) and /setter/leads (setters), each inside
 * their own Shell. All API calls hit /api/sales/leads* regardless of which
 * role is using it - those routes accept both 'sales' and 'setter' via
 * REP_TOOL_ROLES.
 *
 * `scrapeHref` points at the caller's own scraper page. `leadDetailBase`
 * is optional: when omitted (as for setters, who don't have a lead detail
 * page yet - see plan's explicit scope boundary), the business name
 * renders as plain text instead of a link.
 */
export function LeadsWorkspace({
  scrapeHref = '/sales/leads/scrape',
  leadDetailBase,
}: {
  scrapeHref?: string
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2.5">
                      ...
                      {leadDetailBase ? (
                        <Link href={`${leadDetailBase}/${l.id}`} className="flex-1 min-w-0 block group">
                          {nameBlock}
                        </Link>
                      ) : nameBlock}
                    ...
                    {l.phone && (
                      <a
                        href={`tel:${l.phone}`}
                        className="sm:hidden inline-flex items-center gap-1.5 mt-2 text-sm font-mono tabular-nums text-gray-700 select-all"
                      >
