...
function JobDrawer({
 ...
 const unpromoted = useMemo(() => results.filter((r) => !r.promoted_lead_id), [results])
 const allUnpromotedSelected = unpromoted.length > 0 && unpromoted.every((r) => selected.has(r.id))
 ...
 const toggleAll = () => {
  if (allUnpromotedSelected) setSelected(new Set())
  else setSelected(new Set(unpromoted.map((r) => r.id)))
 }
 ...
 return (
  ...
  >
   ...
   <motion.aside
    ...
    className="relative bg-[#0c0c10] border-l border-white/[0.06] w-full max-w-2xl h-full overflow-y-auto shadow-2xl flex flex-col"
   >
    {/* Drawer header */}
     ...
     {loading ? (
      ...
      <>
       <div className="flex items-center justify-between">
        <button
         onClick={toggleAll}
         disabled={unpromoted.length === 0}
         className="text-xs font-mono uppercase tracking-wider text-gray-400 hover:text-white disabled:opacity-50 transition-colors"
        >
         {allUnpromotedSelected ? 'Clear selection' : `Select all unpromoted (${unpromoted.length})`}
        </button>
        <div className="text-xs text-gray-500 font-mono">{selected.size} selected</div>
       </div>
       ...
       <div className="bg-[#101015] border border-white/[0.06] rounded-2xl divide-y divide-white/[0.04] overflow-hidden">
        {results.map((r) => {
         ...
         return (
          <div key={r.id} className="flex items-start gap-3 px-4 py-3 hover:bg-white/[0.02] transition-colors">
           <input
            type="checkbox"
            disabled={promoted}
            checked={isSelected}
           ...
           <div className="flex-1 min-w-0">
            <div className="flex items-baseline justify-between gap-2 flex-wrap">
             ...
             {promoted && (
              <Link
               href={`/admin/leads`}
               className="text-[10px] font-mono uppercase tracking-wider text-emerald-300 bg-emerald-400/10 border border-emerald-400/20 rounded-full px-2 py-0.5 inline-flex items-center gap-1"
              >
               promoted
              </Link>
             )}
    ...
    {/* Sticky footer */}
    {results.length > 0 && (
     <div className="px-6 py-4 border-t border-white/[0.06] sticky bottom-0 bg-[#0c0c10] flex items-center justify-between gap-3">
      ...
      <PrimaryButton
       ...
       loading={promoting}
      >
       <CheckCircle2 className="w-4 h-4" /> Promote selected → leads
      </PrimaryButton>
     </div>
    )}
   </motion.aside>
