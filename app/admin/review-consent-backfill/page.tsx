...
/**
 * One-click backfill: adds the review_consent boolean parameter to the
 * book_appointment tool on every existing Retell agent. Idempotent.
 */
export default function ReviewConsentBackfillPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await fetchWithAuth('/api/admin/agents/backfill-review-consent-tool', {
        method: 'POST',
      })
    ...
    <AdminShell activeLabel="Tools">
      <div className="px-5 sm:px-8 py-6 sm:py-8 max-w-3xl">
        <div className="mb-6">
          ...
          <h1 className="text-2xl font-medium tracking-tight text-white">Review consent tool param</h1>
          <p className="text-sm text-gray-400 mt-1.5">
            Adds <span className="font-mono text-gray-300">review_consent</span> (boolean) to the <span className="font-mono text-gray-300">book_appointment</span> tool on every existing Retell agent so the AI can pass the caller&apos;s answer when scheduling. Idempotent - re-running is safe.
          </p>
        </div>
          ...
          <div className="space-y-4">
            <div className="text-xs text-gray-400 leading-relaxed">
              ...
              <ol className="list-decimal list-inside mt-1.5 space-y-0.5 ml-2">
                <li>GET its agent → response_engine.llm_id</li>
                <li>GET the LLM → general_tools</li>
                <li>Find <span className="font-mono">book_appointment</span></li>
                <li>If <span className="font-mono">review_consent</span> isn&apos;t in <span className="font-mono">parameters.properties</span>, add it</li>
                <li>PATCH the LLM with the new tools array</li>
              </ol>
              <p className="mt-2">Skips agents that don&apos;t use a Retell-managed LLM, that have no <span className="font-mono">book_appointment</span> tool, or whose tool schema doesn&apos;t match expectations.</p>
            </div>
            ...
            {result && (
              <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/[0.06] p-4 space-y-3">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
                  ...
                  <Stat label="Errors" value={(result.summary?.errors || 0) + (result.summary?.unexpected_schema || 0) + (result.summary?.no_book_appointment_tool || 0) + (result.summary?.no_llm || 0)} accent={(result.summary?.errors || 0) > 0 ? 'rose' : 'gray'} />
                </div>
