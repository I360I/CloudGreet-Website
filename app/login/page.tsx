  ...
  const handleLogin = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Store business_id instead of full business object
        if (result.data.user.business_id) {
          localStorage.setItem('business_id', result.data.user.business_id)
        }
        ...
        <div className="text-center mb-8">
          <Link href="/landing" className="inline-block">
            ...
            >
              ...
              <span className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-purple-400 to-blue-400">CloudGreet</span>
            </motion.div>
          </Link>
        ...
        <div className="bg-gray-800/30 backdrop-blur-lg rounded-2xl p-8 border border-gray-700/50">
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full bg-gradient-to-r from-purple-600 to-blue-600 text-white py-4 rounded-xl font-semibold hover:from-purple-700 hover:to-blue-700 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center shadow-lg"
            >
              {isLoading ? (
                <>
